...
```{r}
path_main = "data/ICPRB7/JonesFalls_Test"

path_ssn = paste0(path_main, "/lsn.ssn")


path_preds = paste0(path_main, "/data/Preds_data_JF.txt")

path_sites = paste0(path_main, "/data/Sites_data_JF.txt")
```
...
```{r}
#add sites
ssn_main <- putSSNdata.frame(ssn_joined_sites.df , ssn_main)

#add preds
ssn_main <- putSSNdata.frame(ssn_joined_preds.df, ssn_main)

```


note: SDATE is our sample date and run it by Twat90
...
jpeg("data/ICPRB7/plots/afvArea.jpg")
plot(ssn_main, lwdLineCol = "afvArea", lwdLineEx = 10, lineCol = "blue",
pch = 19, xlab = "x-coordinate (m)", ylab = "y-coordinate (m)",
asp = 1)
```
...
```{r}
#save plot to directory
jpeg("data/ICPRB7/plots/brks.jpg")
brks <- plot(ssn_main, "TWAT90", lwdLineCol = "afvArea",
lwdLineEx = 15, lineCol = "black", xlab = "x-coordinate" ,
ylab = "y-coordinate", asp=1 )
```
...
cex = as.SpatialPointsDataFrame(ssn_main)$TWAT90/9 , add = TRUE)
...
```{r}
#save plot to directory
jpeg("data/ICPRB7/plots/spatial.jpg")
plot(as.SpatialLines(ssn_main), col = "blue")
plot(as.SpatialPoints(ssn_main), pch = 19,
cex = as.SpatialPointsDataFrame(ssn_main)$TWAT90/9 , add = TRUE)
plot(as.SpatialPoints(ssn_main, data = "preds"), cex = 1.5, add = TRUE)
...
```{r}
ssn_main.Torg <- Torgegram(ssn_main, "TWAT90", nlag = 20, maxlag = 50000)
...
jpeg("data/ICPRB7/plots/torg.jpg")
plot(ssn_main.Torg)
```

# 5.2. Model fitting

run glmssn function on our ssn object, for TWAT90, accounting for ELEV and SLOPE covariates and outputs a summary
```{r}
ssn_main.glmssn0 <- glmssn(TWAT90 ~ ELEV + SLOPE, ssn_main,
...
```{r}
#comparison
summary(lm(TWAT90 ~ ELEV + SLOPE, getSSNdata.frame(ssn_main)))

```
"A spatial model, including a mixture of tail-up, tail-down, and Euclidean covariance models
is fitted:"
```{r}
ssn_main.glmssn1 <- glmssn(TWAT90 ~ ELEV + SLOPE, ssn_main,
...
```{r}
#output graphic to directory
jpeg("data/ICPRB7/plots/resid_plot.jpg")
...
```{r}
#output graphic to directory
jpeg("data/ICPRB7/plots/resid_hist.jpg")
par(mfrow = c(1, 2))
hist(ssn_main.resid1)
hist(ssn_main, "TWAT90")
```
plot to markdown
```{r}
par(mfrow = c(1, 2))
...
ObsDF[indOutlier, "TWAT90"] <- NA
ssn_mainc <- putSSNdata.frame(ObsDF, ssn_main)
```

```{r}
ssn_mainc.glmssn0 <- glmssn(TWAT90 ~ ELEV + SLOPE, ssn_mainc,
CorModels = c("Exponential.tailup", "Exponential.taildown",
...
```{r}
ssn_mainc.glmssn1 <- glmssn(TWAT90 ~ ELEV + SLOPE, ssn_mainc,
CorModels = c("Exponential.tailup", "Exponential.taildown"),
...
abline(0, 1)
plot( na.omit( getSSNdata.frame(ssn_mainc)[, "TWAT90"]),
cv.out[, "cv.se"], pch = 19,
...
save plot to directory
```{r}
jpeg("data/ICPRB7/plots/crossvalidation.jpg")
par(mfrow = c(1, 2))
plot(ssn_mainc.glmssn1$sampinfo$z,
cv.out[, "cv.pred"], pch = 19,
xlab = "Observed Data", ylab = "LOOCV Prediction")
...
```{r}
ssn_mainc.glmssn1 <- glmssn(TWAT90 ~ ELEV, ssn_mainc,
CorModels = c("Exponential.tailup", "Exponential.taildown"),
addfunccol = "afvArea", EstMeth = "ML")

######below glmssn can be run when we know the first one runs############

# ssn_mainc.glmssn2 <- glmssn(TWAT90 ~ ELEV, ssn_mainc,
# CorModels = c("LinearSill.tailup", "Mariah.taildown"),
# addfunccol = "afvArea")
# 
# ssn_mainc.glmssn3 <- glmssn(TWAT90 ~ ELEV, ssn_mainc,
# CorModels = c("Mariah.tailup", "LinearSill.taildown"),
# addfunccol = "afvArea")
# 
# ssn_mainc.glmssn4 <- glmssn(TWAT90 ~ ELEV, ssn_mainc,
# CorModels = c("Spherical.tailup", "Spherical.taildown"),
# addfunccol = "afvArea")
# 
# ssn_mainc.glmssn5 <- glmssn(TWAT90 ~ ELEV, ssn_mainc,
...
save plot to directory
```{r}
jpeg("data/ICPRB7/plots/resid1_hist.jpg")
hist(ssn_mainc.resid1.cv.std)
```
...
save plot to directory
```{r}
jpeg("data/ICPRB7/plots/resid1_torg.jpg")
plot(Torgegram(ssn_mainc.resid1, "_resid_", nlag = 8, maxlag = 25000))
...
```
 ...
 save plot to directory
```{r}
jpeg("data/ICPRB7/plots/predictions.jpg")
plot(ssn_mainc.preds, SEcex.max = 1, SEcex.min = .5/3*2,
breaktype = "user", brks = brks)
...
write.csv(main_glmssn1.df,"data/ICPRB7/dataframes/main_glmssn1.csv")

mainc_glmssn0.df <- getSSNdata.frame(ssn_mainc.glmssn0)
write.csv(mainc_glmssn0.df,"data/ICPRB7/dataframes/mainc_glmssn0.csv")

mainc_glmssn1.df <- getSSNdata.frame(ssn_mainc.glmssn1)
write.csv(mainc_glmssn1.df,"data/ICPRB7/dataframes/mainc_glmssn1.csv")

main_resid1.df <- getSSNdata.frame(ssn_main.resid1)
write.csv(main_resid1.df,"data/ICPRB7/dataframes/main_resid1.csv")

mainc_resid1.df <- getSSNdata.frame(ssn_mainc.resid1)
write.csv(mainc_resid1.df,"data/ICPRB7/dataframes/mainc_resid1.csv")

preds.df <- getSSNdata.frame(ssn_mainc.preds)
write.csv(preds.df,"data/ICPRB7/dataframes/preds.csv")
...
# 
# with(getSSNdata.frame(ssn_main), TWAT90[pid==29]) 
```
